...
export const hasArity = (arity, node) => node.args && node.args.length === arity

export const isAdd = node => _isOp('add', node) && node.args.length > 1
export const isMul = node => _isOp('mul', node) && node.args.length > 1
export const isDiv = node => _isOp('div', node) && hasArity(2, node)
export const isPow = node => _isOp('pow', node) && hasArity(2, node)
export const isNeg = node => _isOp('neg', node) && hasArity(1, node)
export const isPos = node => _isOp('pos', node) && hasArity(1, node)
export const isAbs = node => _isOp('abs', node) && hasArity(1, node)
export const isFact = node => _isOp('fact', node) && hasArity(1, node)
export const isNthRoot = node => _isOp('nthRoot', node) && hasArity(2, node)
...
export const isPolynomialTerm = (node) => {
    if (isRationalNumber(node)) {
        ...
        const positiveExp = isInteger(exponent) ? getValue(exponent) >= 1 : true
