...
export const getNumerator = (node) => isNeg(node) ? getNumerator(node.args[0]) : node.args[0]
export const getDenominator = (node) => isNeg(node) ? getDenominator(node.args[0]) : node.args[1]
...
export const getCoefficientsAndConstants = (node) => {
    ...
    node.args.forEach(arg => {
        if (isNumber(arg) || isConstantFraction(arg)) {
            constants.push(arg)
        } else {
